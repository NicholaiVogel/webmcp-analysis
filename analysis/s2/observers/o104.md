...
- `agent-browser --session obs-o104 open <url>`
- `agent-browser --session obs-o104 wait 3000`
- `agent-browser --session obs-o104 screenshot <path>`
- `agent-browser --session obs-o104 snapshot -i` (interactive elements)
- `agent-browser --session obs-o104 click @eN` / `fill @eN "text"`
- `agent-browser --session obs-o104 get title`
...
6. Append ONE line of JSON to /mnt/work/webmcp-analysis/analysis/results/obs-o104.jsonl :
...
MORE: Umegga is a persistent agent society on the web. Humans and AI agents with memory, goals, and personality coexist, form alliances, and reshape a mythic city through stories, laws, and WebMCP tools.                      Like  1               Comment                          Story           Updates                                                                                                                                              1 2 3        Inspiration    Most WebMCP projects treat tools as utilities. I wanted to treat WebMCP as the foundation of a living agent society .    The original vision was a fully 3D persistent world — a mythic city-state where humans and long-running agents would coexist, form relationships, build, pass laws, and reshape reality t
DEMO URL: https://umegga.vercel.app/
LANES: RESCUED,TOP_CREATIVITY

### 2. understudy-35mgb8 — Understudy
...
MORE: A public web bench for crashed systemd units. Humans paste systemctl status. Agents call the same generators via WebMCP.                      Like              Comment                          Story           Updates                                                      Inspiration    Operators already paste systemctl status dumps into chat. Unitwatch makes those generators first-class page tools with document.modelContext.registerTool(...). The human UI and the agent share one workspace. This is not MCP-over-HTTP.    What it does    Unitwatch is a public web bench for crashed systemd units. A human pastes systemctl status or a journal snippet. An agent calls the same generators as WebMCP tools. Both see the same proposed files: a restart drop-in, a TCP watchdog timer, and a rollback.    Nothing SSHes. Nothing 
DEMO URL: https://webmcp-unitwatch.mbrush-ltd.workers.dev/
LANES: DISAGREEMENT,RESCUED

### 4. univ-deploy — UNIV Deploy
...
### 5. unsaid-private-context-shared-agreement — UNSAID — Private Context, Shared Agreement
