...
- `agent-browser --session obs-o105 open <url>`
- `agent-browser --session obs-o105 wait 3000`
- `agent-browser --session obs-o105 screenshot <path>`
- `agent-browser --session obs-o105 snapshot -i` (interactive elements)
- `agent-browser --session obs-o105 click @eN` / `fill @eN "text"`
- `agent-browser --session obs-o105 get title`
...
6. Append ONE line of JSON to /mnt/work/webmcp-analysis/analysis/results/obs-o105.jsonl :
...
MORE: Agent-assisted saree shopping: search, compare, open products, and add to cart with WebMCP while people keep control of checkout and payment.                      Like              Comment                          Story           Updates                                                  Vaanzari                            Vaanzari                            Vaanzari     1 2        Inspiration    Shopping for a saree often means translating a personal request into filters, comparing several products, opening product pages, and keeping track of a cart. Traditional web agents have to guess at buttons and page structure. We wanted the storefront to expose a clear, predictable interface for people and agents to use together.    What we built    Vaanzari adds browser-native WebMCP tools
DEMO URL: https://vaanzari.com/developers/webmcp
LANES: DISAGREEMENT,RESCUED

### 2. velaire-webmcp-agreements-for-service-businesses — Velaire — WebMCP Agreements for Service Businesses
...
### 4. verb-sign-off-voice-on — Verb - Sight off. Voice on
